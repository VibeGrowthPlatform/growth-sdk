...
public static class VibeGrowthUnityE2ERunner
{
    ...
    public static void Run()
    {
        try
        {
            ...
            Debug.Log("[VibeGrowthUnityE2E] passed");
            EditorApplication.Exit(0);
        }
        catch (Exception error)
        {
            WriteResults("Failed", error.ToString());
            Debug.LogError($"[VibeGrowthUnityE2E] failed: {error}");
    ...
    private static void RunOrThrow()
    {
        ...
        Debug.Log($"[VibeGrowthUnityE2E] initializing deviceId={deviceId}");
        ...
        Debug.Log($"[VibeGrowthUnityE2E] verified deviceId={deviceId} userId={userId} productId={productId}");
    }
    ...
    private static void EventuallyEquals(
        SdkE2eConfig config,
        string expected,
        string query,
    ...
    {
        var path = Environment.GetEnvironmentVariable("VG_UNITY_E2E_RESULTS_PATH");
        if (string.IsNullOrWhiteSpace(path))
        {
            path = Path.Combine(Directory.GetCurrentDirectory(), "TestResults.xml");
        }
        ...
        File.WriteAllText(
            path,
            $"<test-run result=\"{result}\"><test-suite name=\"VibeGrowthUnityE2E\" result=\"{result}\">{failure}</test-suite></test-run>",
            Encoding.UTF8
        );
    }
